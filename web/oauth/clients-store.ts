...
function formatScope(record: OAuthClientRecord): string {
  if (record.scope) return record.scope;
  if (Array.isArray(record.scopes) && record.scopes.length > 0) return record.scopes.join(" ");
...
export class ClientsStore implements OAuthRegisteredClientsStore {
  async getClient(clientId: string): Promise<any | undefined> {
    try {
      ...
      console.error("ClientsStore.getClient error:", err);
      throw err;
    }
  }
